    ...
    <n-layer ref="layer" :layerForm="isLayerForm">
      <template slot="layerImg" v-if='state==0'>
        <div class="clear" style="max-height: 100%;overflow-y: auto;">
          <div class="posa white cusp" style="top:-40px;right:0;"><img style="width:28px;" class="vtm mgr10" src="../assets/imgs/btn_play_share.png">发送至QQ群</div>
          <div class="txl f16 heise mgt10 lineh"><p>项目名称：<span class="ticket_pro_name">海口绕城公路演丰至美兰段 </span>项目</p>
						<p class="posr contenteditable_box">罚款原因：<span class="posa contenteditable contenteditablespan" contenteditable="true">2018年12月23日：</span></p>
						<p style="width:25%;margin-top:21%;" class="posr mgt10 price_box f16">罚款金额：
              <input type="text" value="" class="f16" style="width:40%; background-color: transparent;border-bottom: 1px solid #C3C3C4;height: 24px;color:#000000;">
              <span class="posa" style="right:6%">元</span>
            </p>
          </div>
          <div class="f15 lineh huise2 mgt20">
            <p>罚款依据：</p><p>1.损坏摄像头，罚款5000元/个，并在两小时之内修复。</p>
            <p>2.故意遮挡摄像头，罚款50元/次，每日罚款上限500元。</p><p>3.损坏摄像头电源线，罚款100元/次，并在两小时之内修复，超过1小时加罚50元，每日罚款上限500元。</p>
            <p>4.损坏摄像头网线，罚款100元/次，并在两小时之内修复，超过1小时加罚50元，每日罚款上限500元。</p><p>5.未佩戴安全防护设备，罚款50元/次，每日罚款上限500元。</p>
            <p>备注：客观原因（如：天气恶劣、设备老化、检修等等）造成设备故障，每月三次内不做处罚，超过三次按以上违规现象处理。</p>
          <p class="txr heise f16">项目管理中心</p><p class="txr heise f16">{{common.formatTime('','','','','',1)}}</p></div>
        </div>
      </template>
      <template slot="layerImg" v-if='state==1'>
        <div style="width:100%;height:100%" >
          <img :src="cropperSrc" style="width:100%;height:70%;margin-bottom:3%;" />
          <el-input type="textarea" :rows="4" placeholder="备注" v-model="textarea"></el-input>
          <div class="txc sub-btn-div"><span @click="submitForm" class="sub-btn">确认保存</span></div>
        </div>
      </template>
    </n-layer>
    <!-- <template v-if='listType==1'> -->
    <div class="posa full pd20 bdr20" :style="{zIndex:listType==1?'100':'10',paddingTop:'10px',backgroundColor:listType==1?'white':'transparent'}">
      <div class="f15">
        <n-back :isHide="true" @backFun="recover" class="vtm cusp mgr10 punish-img" v-show="isFull"></n-back>
        <template v-if="lastedNew">
          <img src="../assets/imgs/safe_09.png" class="vtm mgr10 icon-img" >{{lastedNew.date}} {{lastedNew.content}}
        </template>
      <label class="label-More" @click="moreList">智能抓拍</label>
      </div>
      <div :class="['lt','security-video-l','posa','security-video-l_'+countNum]">
        <div class="txc" style="margin-top:20%;" v-if="cameralistjson['cameralist'+countNum].length==0">
          <img class="nodata-img" src="../assets/imgs/default_data.png">
          <p class="heise">暂无数据</p>
        </div>
        <div v-for="(item,index) in cameralistjson['cameralist'+countNum]" :id="isActive==index ? 'intel_item_choose' : ''" :key="index" :class="[isActive==index ? 'intel_item_choose blue' : '','txl','white','posa','intel_item','pd15','intel_item_' +(index+1),isFullArr['cameralist'+countNum][index] ?'popBox':'']" :style="{visibility:(isFull&&!isFullArr['cameralist'+countNum][index]?'hidden':'visible')}">
          <!-- 截屏完成后执行事件 -->
          <span class="corpper" @click="showCorpper"></span>
          <!-- 没有截屏插件提示安装 -->
          <span class="downloadUrl" @click="downloadUrl"></span>
          <p class="posa video_name heise clear f14 posr" @click.prevent="choosevideo(index)">
            <span class="lt">{{item.name}}</span>
            <img src="../../static/img/btn_model_all.png" class="posa cusp" style="right:10px;top:7px;width:35px;height:35px;" v-show="!isFull" @click.prevent="bigvideoFun(item,index)">
          </p>
          <div @click="cropper()" class="video-foot"><img :class="classShot[0]" class="vtm" src='../../static/img/safe_22.png'/><span :class="classShot[1]">截屏</span></div>
      ...
      <div class="rt security-video-r posa bdr20 wtbg posr">
        ...
        <div class="posa wall txc postb0">
          <img class="mgr10 cusp" src="../assets/imgs/safe_12.png" @click="changeFun(4)">
          <img class="mgr10 cusp" src="../assets/imgs/safe_13.png" @click="changeFun(9)">
          <img class="mgr10 cusp" src="../assets/imgs/safe_14.png" @click="changeFun(16)">
        </div>
      </div>
    </div>
    <!-- </template> -->
<!-- <template v-if='listType==2'> -->
   <div class="posa full pd20 bdr20" :style="{zIndex:listType==1?'10':'100',paddingTop:'10px',backgroundColor:listType==1?'transparent':'white'}">
    <n-back :isHide="true" @backFun="backFun" class="vtm"></n-back>
    <n-times @changeTime="changeTime" class="mgl30"></n-times>
     <div class="p-inf-main txc" style="top:50px;">
      <el-table class="mgt20 el-table-target" :data="tableList" style="width: 100%;height:100%;" height="0.1" >
        <el-table-column align="center" type="index" width="120" label="序号" :index="(ind)=> common.indexMethod(ind,$data)"></el-table-column>
        <el-table-column align="center" width="130" label="截图">
        <template slot-scope="scope"><img :src='scope.row.rcontent?scope.row.rcontent:""' style='width:100px;height:50px;cursor:pointer;' @click="lookbidgQrCode(scope.row.rcontent)"/></template>
        </el-table-column>
        <el-table-column align="center" width="240" prop="date"  label="时间"></el-table-column>
        <el-table-column align="center" prop="content"  label="内容"></el-table-column>
      </el-table>
     </div>
    <el-pagination class="m-pagination el-pagination-person" background layout="prev, pager, next" :page-size="10" @current-change="getList" :current-page.sync="currentPage" :total="totalcount"></el-pagination>
  </div>
    <!-- </template> -->
</div>
</template>
...
.contenteditablespan:focus{outline: none;}
.icon-img {
  width: 15px;
}
.punish-img {
...
export default {
  ...
  methods: {
    ...
    QueryNotSafeHatMsgList(){
      this.post(this,'QueryNotSafeHatMsgList',{page:0,section:Number(this.common.localget("section"))}).then(res=>{
    ...
    submitForm(){
      ...
      that.post(that, "UploadMulitFile",fd,{ showLoading: true },true).then(res => {
        if(res.state===0){
          ...
          that.submitSure({rid:this.rid,content:this.textarea,rcontent:fileArr});
        }
      })
    },
    submitSure(obj){
      this.post(this,'UpdateNotSafeHat',obj,{ showLoading: true }).then(res=>{
        let that = this;
        that.bus.$emit("tipFun", { type: 1, msg: "", fun: () => {
    ...
    handleNodeClick(item) {
      ...
      this.$nextTick(function() {
        $(".intel_item_choose").find(".video-foot").next().remove();
        $(".intel_item_choose").find(".video_name span").text(item.name);
        let str ='<video style="width:100%;height:100%;" controls id="myPlayer' +this.isActive +'" poster="'+(item.camerImage?item.camerImage:'https://hkrct.cninct.com/hkrc_dir/default/cam_default_15_1.png')+'"><source src="' +item.rtmp +'" type="" /></video>';
      ...
      for (let i = 0; i < listc.length; i++) {
        if (listc[i].ischoose) {
          ...
          this.$set(this.cameralistjson["cameralist" + this.countNum][i],"ischoose",false);
        }
      }
    ...
    QuerySafeCameraList() {
      this.post(this, "QuerySafeCameraList", {section:Number(this.common.localget("section"))},{ showLoading: true }).then(res => {
    ...
    arrrayFun() {
      ...
      this.$nextTick(function() {
        ...
        for (let i = 0;i < this.cameralistjson["cameralist" + this.countNum].length;i++) {
          str ='<video style="width:100%;height:100%;" id="myPlayer' + i +'" poster="'+(this.cameralistjson["cameralist" + this.countNum][i].camerImage?this.cameralistjson["cameralist" + this.countNum][i].camerImage:'https://hkrct.cninct.com/hkrc_dir/default/cam_default_15_1.png')+'"><source src="' +this.cameralistjson["cameralist" + this.countNum][i].rtmp +'" type="" /></video>';
